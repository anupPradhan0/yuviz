...
class TestAgentEndpoints:
    async def test_create_and_get_agent(self, client, test_tenant):
        ...
        assert "workflow" in created and "workflow_draft" in created
        assert isinstance(created["workflow"], dict)
        assert created["workflow_draft"] == created["workflow"]

        resp = await client.get(f"/tenants/{test_tenant['slug']}/agents/support-agent")
        assert resp.status_code == 200
        body = resp.json()
        assert body["greeting"] == "Hi!"
        assert "workflow" in body and "workflow_draft" in body
        ...
        assert body["workflow_draft"] == graph
