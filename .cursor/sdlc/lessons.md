...
and complies with the lessons tagged for it. Append-only via `/sdlc-retro` (Cursor) or `/sdlc:retro`
(Claude); keep each entry to two lines. Delete an entry only when it becomes wrong, not when it
becomes familiar.

Tags: [prd] [architect] [planner] [implementer] [critic] [security] [tester] [all]
